...
class SQLUtil
{
	...
	public long currValReservas (PersistenceManager pm)
	{
        Query q = pm.newQuery(SQL, "SELECT "+ pp.darSeqReservas() + ".nextval FROM DUAL");
        q.setResultClass(Long.class);
        long resp = (long) q.executeUnique();
        return resp;
	}
	...
	public long currValServicioSalud(PersistenceManager pm) {
		 	Query q = pm.newQuery(SQL, "SELECT "+ pp.darSeqServicioSalud() + ".nextval FROM DUAL");
	        q.setResultClass(Long.class);
	        long resp = (long) q.executeUnique();
	        return resp;
	}
