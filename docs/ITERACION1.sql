...
FROM ALL_TRIGGERS 
  WHERE OWNER = '' 
  AND TRIGGER_NAME = 'TRG_ORDENES_SERVICIOS_ID'; 
  IF (C > 0) THEN 
    EXECUTE IMMEDIATE 'DROP TRIGGER "TRG_ORDENES_SERVICIOS_ID"'; 
END IF; 
END 
...
FROM ALL_SEQUENCES 
  WHERE SEQUENCE_OWNER = '' 
  AND SEQUENCE_NAME = 'SEQ_ORDENES_SERVICIOS_ID'; 
  IF (C > 0) THEN 
    EXECUTE IMMEDIATE 'DROP SEQUENCE "SEQ_ORDENES_SERVICIOS_ID"'; 
END IF; 
END 
...
DROP SEQUENCE "SEQ_CONSULTA_URGENCIAS_ID";
DROP SEQUENCE "SEQ_EPS_ANDES_ID";
DROP SEQUENCE  "SEQ_HORARIO_ID";
DROP SEQUENCE   "SEQ_IPS_ID";
DROP SEQUENCE   "SEQ_MEDICO_ID";
DROP SEQUENCE   "SEQ_ORDEN_ID";
DROP SEQUENCE  "SEQ_ORDENES_SERVICIOS_ID";
DROP SEQUENCE  "SEQ_RESERVAS_ID";
DROP SEQUENCE   "SEQ_ROL_ID";
DROP SEQUENCE  "SEQ_SERVICIO_SALUD_ID";
...
(
	"ID" NUMBER(8) NOT NULL,
	"IDAFILIADOTOMADOR" NUMBER(8),
	"IDHORARIO" NUMBER(8) NOT NULL,
	"IDAFILIADORESERVADOR" NUMBER(8) NOT NULL,
...
CREATE SEQUENCE "SEQ_ORDENES_SERVICIOS_ID" 
	INCREMENT BY 1 
	START WITH 1 
	NOMAXVALUE 
	MINVALUE  1 
...
CREATE OR REPLACE TRIGGER "TRG_ORDENES_SERVICIOS_ID" 
	BEFORE INSERT 
	ON "ORDENES_SERVICIOS" 
	FOR EACH ROW 
	BEGIN 
		SELECT "SEQ_ORDENES_SERVICIOS_ID".NEXTVAL 
		INTO :NEW."ID" 
		FROM DUAL; 
	END
;
...
ALTER TABLE "HORARIO" 
 ADD CONSTRAINT "CK_HORARIO_DISPONIBILIDAD" CHECK (DISPONIBILIDAD IN (0,1))
;
...
CREATE INDEX "IXFK_RESERVAS_AFILIADO"   
 ON "RESERVAS" ("IDAFILIADOTOMADOR") 
;

CREATE INDEX "IXFK_RESERVAS_AFILIADO02"   
 ON "RESERVAS" ("IDAFILIADORESERVADOR") 
;

CREATE INDEX "IXFK_RESERVAS_HORARIO"   
 ON "RESERVAS" ("IDHORARIO") 
;
...
ALTER TABLE "RESERVAS" 
 ADD CONSTRAINT "CK_RESERVAS_HOSPITALIZADO" CHECK (HOSPITALIZADO IN (0,1))
;
...
ALTER TABLE "RESERVAS" 
 ADD CONSTRAINT "FK_RESERVAS_AFILIADO"
	FOREIGN KEY ("IDAFILIADOTOMADOR") REFERENCES "AFILIADO" ("IDUSUARIO")
;

ALTER TABLE "RESERVAS" 
 ADD CONSTRAINT "FK_RESERVAS_AFILIADO02"
	FOREIGN KEY ("IDAFILIADORESERVADOR") REFERENCES "AFILIADO" ("IDUSUARIO")
;
